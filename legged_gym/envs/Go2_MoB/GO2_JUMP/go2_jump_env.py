...
class GO2_JUMP_Robot(BaseTask):
    ...
    def update_cmd_action_latency_buffer(self):
        # 使用在 _compute_torques 里计算好的进度 (如果没有就默认0)
        progress = getattr(self, 'curriculum_factor', 0.0)

        # ================================================================
        # Action Scale 动态增大 (Scale Up)
        # ================================================================
        # 初始 10.0 -> 目标 25.0
        # 这样后期网络才有足够的能力去支撑身体
        initial_scale = 10.0 # 对应 Config 中的配置
        target_scale = 23.5 
        
        current_scale = initial_scale + progress * (target_scale - initial_scale)

        # 应用动态缩放
        actions_scaled = self.actions * current_scale
    ...
    def compute_observations(self):
        ...
        obs_buf = torch.cat((
            ...
            self.actions,   # 12
            self.torques,   # 12
            torch.ones(self.num_envs, 1, device=self.device) * getattr(self, 'curriculum_factor', 0.0), # 1: PD Decay Progress
        ), dim=-1)
    ...
    def _compute_torques(self, actions):
        ...
        if self.common_step_counter < warmup_steps:
            ...
            progress = np.clip((self.common_step_counter - warmup_steps) / ramp_steps, 0.0, 1.0)

        # [关键] 将进度存为全局变量，供其他函数（奖励、ActionScale）使用
        self.curriculum_factor = progress
        # print(f"DEBUG: Step={self.common_step_counter}, Progress={progress:.4f}, PD_Factor={1.0-progress:.4f}")

        # ================================================================
        # 2. PD 动态衰减 (PD Fade Out)
        # ================================================================
        # 0.0 (Pre-warm) -> 1.0 (Full Torque)
        # 随着 progress 变大，pd_factor 变小
        pd_factor = 1.0 - progress 
        
        # 实时调整 PD Gains
        p_gains = self.p_gains * self.p_gains_multiplier * pd_factor
        d_gains = self.d_gains * self.d_gains_multiplier * pd_factor
        ...
        # 叠加策略网络输出 (actions 已经在 update_cmd_action_latency_buffer 里被放大了)
    ...
    def _reward_orientation(self):
        # 1. 原始奖励
        raw_reward = torch.exp(-torch.norm(self.projected_gravity[:, :2], dim=1)*10)
        
        # 2. 动态增强 (Dynamic Boost for Stability)
        # 当 PD 消失时 (progress -> 1.0)，大幅增加姿态奖励权重
        # 基础权重 2.0 * Boost 2.5 = 最终权重 5.0
        progress = getattr(self, 'curriculum_factor', 0.0)
        boost = 1.0 + progress * 1.5 
        
        return raw_reward * boost
    ...
    def _reward_base_height(self):
        ...
        raw_reward = torch.exp(-torch.abs(base_height - self.cfg.rewards.base_height_target)*10)
