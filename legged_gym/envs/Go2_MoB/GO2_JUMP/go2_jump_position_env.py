...
# [修改] 类名重命名为 GO2_JUMP_Position_Robot
class GO2_JUMP_Position_Robot(BaseTask):
    # [修改] 类型注解更新
    def __init__(self, cfg: GO2_JUMP_Position_Cfg, sim_params, physics_engine, sim_device, headless):
        self.cfg = cfg
    ...
    def reset_idx(self, env_ids):
        ...
        if self.cfg.terrain.curriculum: self._update_terrain_curriculum(env_ids)
    ...
    def compute_observations(self):
        ...
        self.privileged_obs_buf = torch.cat((
            ...
            self.base_euler_xyz *self.cfg.normalization.obs_scales.quat,
            self.env_frictions,
            self.body_mass / 10.,
            stance_mask,
            contact_mask,    
    ...
    def _reset_root_states(self, env_ids):
        if self.custom_origins:
            ...
            self.root_states[env_ids, :2] += torch_rand_float(-4., 4., (len(env_ids), 2), device=self.device) 
        else:
            self.root_states[env_ids] = self.base_init_state
            self.root_states[env_ids, :3] += self.env_origins[env_ids]
    ...
    def _push_robots(self):
        ...
        self.root_states[:, 10:13] = torch_rand_float(-max_push_angular, max_push_angular, (self.num_envs, 3), device=self.device) 
    ...
    def _draw_debug_vis(self):
        if not self.terrain.cfg.measure_heights: return
